...
#endif
#ifndef TR456_WATER_REFLECT_ORIGINAL_MASK
#define TR456_WATER_REFLECT_ORIGINAL_MASK 0.0
#endif
...
#define TR_TOGGLE_FLOW_ENABLE uTrWaterToggle0.x
#define TR_TOGGLE_FLOW_WARP uTrWaterToggle0.y
#define TR_TOGGLE_FLOW_REFLECTION uTrWaterToggle0.z
#define TR_TOGGLE_FLOW_FOAM uTrWaterToggle0.w
#define TR_TOGGLE_FLOW_LANES uTrWaterToggle1.x
#define TR_TOGGLE_FLOW_GLINT uTrWaterToggle1.y
#define TR_TOGGLE_FLOW_TENSION uTrWaterToggle1.z
#define TR_TOGGLE_FLOW_DETAIL uTrWaterToggle1.w
#define TR_TOGGLE_FLOW_CONTACTS uTrWaterToggle2.x
#define TR_TOGGLE_SURFACE_FOAM uTrWaterToggle2.y
#define TR_TOGGLE_SURFACE_REFLECTION uTrWaterToggle2.z
#define TR_TOGGLE_CONTACT_RIPPLES uTrWaterToggle2.w
#define TR_TOGGLE_FLOW_CHROMA TR_TOGGLE_FLOW_WARP
...
vec3 trshaderReliefField(vec2 trshaderW, vec2 trshaderUv, float trshaderT, vec2 trshaderPrimaryDir){
 ...
 float trshaderStrength=clamp(TR456_WATER_SURFACE_RELIEF*TR456_WATER_TEXTURE_STRENGTH,0.0,3.10)*.38;
...
vec4 trshaderRenderFastFlow(vec2 trshaderScreen, float trshaderT){
 if(TR_TOGGLE_FLOW_ENABLE<=.001) return vec4(0.0);
 ...
 float trshaderSceneScale=clamp(uTrWaterSyntheticMode.z,0.0,1.0);
 vec2 trshaderWarp=(trshaderScreenDir*(trshaderLong*.0038+trshaderFine*.0020)+
   trshaderScreenSide*(trshaderCross*.0030))*trshaderWarpAmount*trshaderSceneScale;
 ...
 float trshaderGlint=pow(trshaderSat(trshaderLine*trshaderRibbon),18.0)*
   TR456_WATER_GLINT_STRENGTH*TR456_WATER_FLOW_GLINT*TR_TOGGLE_FLOW_GLINT;
...
vec2 trshaderSyntheticStandingBump(vec2 trshaderBaseSlope, vec2 trshaderReliefSlope,
 ...
 return trshaderSyntheticBumpSlope(trshaderBaseSlope*.34+trshaderReliefSlope*1.55+
...
vec3 trshaderOriginalUnderlayColor(vec2 trshaderScreen, vec3 trshaderFallback){
 if(uTrWaterUnderlayInfo.z<=.001) return trshaderFallback;
 vec2 trshaderUv=clamp(trshaderScreen,vec2(.001),vec2(.999));
 vec2 trshaderStep=max(uTrWaterUnderlayInfo.xy,vec2(1.0/8192.0));
 vec3 trshaderUnder=texture(uTrWaterUnderlay,trshaderUv).rgb;
 vec3 trshaderUnderA=texture(uTrWaterUnderlay,
   clamp(trshaderUv+trshaderStep*vec2(4.0,0.0),vec2(.001),vec2(.999))).rgb;
 vec3 trshaderUnderB=texture(uTrWaterUnderlay,
   clamp(trshaderUv+trshaderStep*vec2(0.0,4.0),vec2(.001),vec2(.999))).rgb;
 vec3 trshaderSmoothed=trshaderUnder*.70+trshaderUnderA*.15+trshaderUnderB*.15;
 return trshaderOriginalWaterGrade(mix(trshaderUnder,trshaderSmoothed,.25));
}

float trshaderReflectOriginalMask(vec2 trshaderScreen){
 float trshaderStrength=clamp(TR456_WATER_REFLECT_ORIGINAL_MASK,0.0,1.0);
 float trshaderReflectPass=1.0-smoothstep(.20,.80,abs(uTrWaterSyntheticMode.y-2.0));
 if(trshaderStrength<=.001 || trshaderReflectPass<=.001)
  return 1.0;
 if(uTrWaterUnderlayInfo.z<=.001)
  return 1.0;
 vec2 trshaderUv=clamp(trshaderScreen,vec2(.001),vec2(.999));
 vec2 trshaderStep=max(uTrWaterUnderlayInfo.xy,vec2(1.0/8192.0));
 vec2 trshaderDx=vec2(trshaderStep.x*2.0,0.0);
 vec2 trshaderDy=vec2(0.0,trshaderStep.y*2.0);
 vec3 trshaderScene=texture(uTrWaterScene,trshaderUv).rgb;
 vec3 trshaderUnder=texture(uTrWaterUnderlay,trshaderUv).rgb;
 vec3 trshaderSceneX=texture(uTrWaterScene,clamp(trshaderUv+trshaderDx,vec2(.001),vec2(.999))).rgb;
 vec3 trshaderUnderX=texture(uTrWaterUnderlay,clamp(trshaderUv+trshaderDx,vec2(.001),vec2(.999))).rgb;
 vec3 trshaderSceneY=texture(uTrWaterScene,clamp(trshaderUv+trshaderDy,vec2(.001),vec2(.999))).rgb;
 vec3 trshaderUnderY=texture(uTrWaterUnderlay,clamp(trshaderUv+trshaderDy,vec2(.001),vec2(.999))).rgb;
 float trshaderSignal=length(trshaderUnder-trshaderScene);
 trshaderSignal=max(trshaderSignal,abs(trshaderLuma(trshaderUnder)-trshaderLuma(trshaderScene))*1.65);
 float trshaderSignalX=length(trshaderUnderX-trshaderSceneX);
 float trshaderSignalY=length(trshaderUnderY-trshaderSceneY);
 trshaderSignal=max(trshaderSignal,max(trshaderSignalX,trshaderSignalY)*.78);
 float trshaderMask=smoothstep(.0007,.0105,trshaderSignal);
 trshaderMask=clamp(trshaderMask*1.16+.10,0.0,1.0);
 trshaderMask=max(trshaderMask,.58);
 return mix(1.0,trshaderMask,trshaderStrength*trshaderReflectPass);
}
...
vec4 trshaderSyntheticFlowPattern(vec2 trshaderP, float trshaderTime, float trshaderSpeed, vec4 trshaderPatchField){
 ...
 float trshaderCrossStrength=clamp(TR456_WATER_FLOW_CROSS_WAVE,0.0,2.0)*TR_TOGGLE_FLOW_TENSION;
 ...
 float trshaderStream=max(trshaderLineMask(trshaderStreamCoord,9.5)*.28,
   trshaderLineMask(trshaderFineCoord,19.0)*.38);
 ...
 float trshaderCrossWave=(trshaderLineMask(trshaderCrossCoord,7.5)*.26+
   trshaderLineMask(trshaderCrossFine,16.0)*.18)*trshaderCrossStrength*TR_TOGGLE_FLOW_LANES;
 float trshaderLifePower=clamp(TR456_WATER_FLOW_DETAIL,0.0,2.0)*TR_TOGGLE_FLOW_DETAIL;
 ...
 float trshaderCrest=trshaderSat(trshaderStream*.30+trshaderLane*.30+trshaderRibbon*.32+trshaderStreak*.28+
   ...
   abs(trshaderChop)*.11*TR456_WATER_FLOW_BREAKUP);
...
vec3 trshaderSyntheticFlowField(vec2 trshaderP, float trshaderTime, float trshaderSpeed, vec4 trshaderPatchField){
 ...
 float trshaderH=trshaderMain*.20+trshaderLongWave*.11+trshaderSideTurb*.08+trshaderFast*.12+
   (trshaderCrossMain*.12+trshaderCrossFast*.075)*trshaderCrossStrength+
   trshaderPatchCue;
 vec2 trshaderSlope=vec2(
   ...
   cos(trshaderQ.x*78.0+trshaderN0*2.5-trshaderAdv*(1.34+trshaderSpeed*.28))*.006+
   cos(trshaderQ.y*24.0+trshaderQ.x*2.3+trshaderN0*1.4-trshaderAdv*(.50+trshaderSpeed*.09))*.004*trshaderCrossStrength,
   cos(trshaderQ.x*18.0+trshaderQ.y*1.15+trshaderN1*1.7-trshaderAdv*.48)*.005+
   cos(trshaderQ.x*14.5+trshaderQ.y*2.4+trshaderN1*1.2-trshaderAdv*(.42+trshaderSpeed*.10))*.006+
   (cos(trshaderQ.y*24.0+trshaderQ.x*2.3+trshaderN0*1.4-trshaderAdv*(.50+trshaderSpeed*.09))*.017+
    cos(trshaderQ.y*54.0-trshaderQ.x*1.6+trshaderN1*2.0-trshaderAdv*(.88+trshaderSpeed*.18))*.008)*trshaderCrossStrength);
...
vec3 trshaderFlowMicroChopField(vec2 trshaderP, float trshaderTravel, float trshaderSpeed, vec4 trshaderPatchField){
 ...
 vec2 trshaderSlope=vec2(cos(trshaderPhaseA)*.010+cos(trshaderPhaseB)*.006+
   cos(trshaderPhaseC)*.003*trshaderCrossStrength,
   (trshaderN0-trshaderN1)*.006+cos(trshaderPhaseA*.63+trshaderPhaseB*.18)*.003+
   cos(trshaderPhaseC)*.011*trshaderCrossStrength)*trshaderAmp;
 float trshaderChop=trshaderSat((abs(sin(trshaderPhaseA))*.42+abs(sin(trshaderPhaseB))*.24+
   abs(sin(trshaderPhaseC))*.20*trshaderCrossStrength+trshaderPatchField.y*.18)*trshaderAmp);
...
vec4 trshaderStandingLifeField(vec3 trshaderW, float trshaderT,
 ...
 float trshaderTremble=clamp(TR456_WATER_STANDING_TREMBLE,0.0,3.0);
 float trshaderBreath=clamp(TR456_WATER_STANDING_BREATH,0.0,3.0);
 ...
 float trshaderTrembleA=sin(trshaderP.x*34.0+trshaderPatch.x*2.4+
   trshaderDrift*7.2+sin(trshaderP.y*19.0-trshaderDrift*2.1)*.58);
 float trshaderTrembleB=sin(trshaderP.y*47.0-trshaderPatch.z*2.1-
   trshaderDrift*9.6+sin(trshaderP.x*16.0+trshaderDrift*2.7)*.46);
 float trshaderTrembleC=sin((trshaderP.x-trshaderP.y)*58.0+
   trshaderDrift*13.4+trshaderTrembleA*.40);
 float trshaderBreathA=sin(trshaderP.x*1.92+trshaderPatch.y*.64+
   trshaderDrift*.42+sin(trshaderP.y*1.10-trshaderDrift*.18)*.55);
 float trshaderBreathB=sin(trshaderP.y*2.45-trshaderPatch.w*.52-
   trshaderDrift*.36+sin(trshaderP.x*.86+trshaderDrift*.14)*.48);
 ...
 vec2 trshaderTrembleSlope=(trshaderPrimaryDir*
   (trshaderTrembleA*.62+trshaderTrembleC*.22)+trshaderSideDir*
   (trshaderTrembleB*.58-trshaderTrembleC*.20))*trshaderTremble;
 vec2 trshaderBreathSlope=(trshaderPrimaryDir*
   (trshaderBreathA*.34+trshaderBreathB*.16)+trshaderSideDir*
   (trshaderBreathB*.30-trshaderBreathA*.12))*trshaderBreath;
  vec2 trshaderSlope=(trshaderFlowSlope*.30+trshaderChopSlope*.80*trshaderMicro+
   trshaderStreakSlope*.58+trshaderVolumeSlope*.44+
   trshaderTrembleSlope*.34+trshaderBreathSlope*.20+
 ...
 float trshaderEnergy=trshaderSat(abs(trshaderFlow.z)*.32+
   ...
   (abs(trshaderTrembleA)+abs(trshaderTrembleB))*.060*trshaderTremble+
   (abs(trshaderBreathA)+abs(trshaderBreathB))*.028*trshaderBreath+
...
TrshaderSyntheticFrame trshaderBuildSyntheticFrame(vec2 trshaderScreen, float trshaderT){
 ...
 float trshaderFlowRippleGate=step(.5,TR456_WATER_FLOW_CONTACT_RIPPLES)*TR_TOGGLE_FLOW_CONTACTS;
 ...
 if(trshaderFlowProfile>.001 && TR_TOGGLE_FLOW_CONTACTS>.001) {
 ...
 float trshaderReliefSlopeStrength=mix(.56,.80,trshaderStandingProfile);
...
TrshaderSyntheticFrame trshaderStandingPoolReplacementFrame(TrshaderSyntheticFrame trshaderF){
 ...
 trshaderP.trshaderWaterfallWaves=trshaderF.trshaderWaterfallWaves;
  trshaderP.trshaderSlope=trshaderP.trshaderBaseField.xy*.78+trshaderP.trshaderContacts.xy*1.65+trshaderP.trshaderRainRipples.xy*1.80+
   ...
   trshaderP.trshaderRelief.xy*.80+trshaderP.trshaderAlive.xy*.48+
...
vec4 trshaderRenderCascadeFlow(TrshaderSyntheticFrame trshaderF, vec2 trshaderFlowDir, vec2 trshaderFlowSide,
 ...
 vec2 trshaderTexFlow=length(vSynFlowDir)>.000001 ? normalize(vSynFlowDir) : vec2(0.0,-1.0);
...
TrshaderFlowCore trshaderBuildFlowCore(float trshaderFlowTime,
 ...
 trshaderCore.trshaderFlowField=trshaderSyntheticFlowField(
   trshaderCore.trshaderFlowUv,trshaderFlowTime,trshaderFlowSpeed,
   trshaderCore.trshaderFlowAnimPatch);
 trshaderCore.trshaderMicroChop=vec3(0.0);
 trshaderCore.trshaderRefrStreak=vec4(0.0);
 if(TR_TOGGLE_FLOW_DETAIL>.001) {
...
vec4 trshaderRenderSurfaceFlow(TrshaderSyntheticFrame trshaderF, vec2 trshaderFlowDir, vec2 trshaderFlowSide,
 ...
 vec4 trshaderRefrStreak=trshaderCore.trshaderRefrStreak;
  vec4 trshaderVolumeWave=TR_TOGGLE_FLOW_TENSION>.001 ?
    trshaderFlowVolumeWaveField(trshaderFlowUv,trshaderGameTravel,trshaderFlowSpeed,trshaderPatchField) : vec4(0.0);
  vec4 trshaderSmoothDeform=TR_TOGGLE_FLOW_DETAIL>.001 ?
    trshaderSmoothFlowDeformationField(vSynWorldPos,trshaderFlowDir,trshaderFlowSide,
      trshaderGameTravel,trshaderFlowSpeed,0.0) : vec4(0.0);
 ...
 float trshaderCrossShear=(trshaderCrossShearA*.55+trshaderCrossShearB*.45)*
   trshaderCrossStrength*trshaderCrossDistortion;
 ...
 float trshaderTensionStrength=clamp(TR456_WATER_FLOW_SURFACE_TENSION,0.0,2.0)*TR_TOGGLE_FLOW_TENSION;
 ...
 float trshaderFlowContactStrength=clamp(TR456_WATER_FLOW_CONTACT_STRENGTH,0.0,3.0)*
    TR_TOGGLE_FLOW_CONTACTS;
  float trshaderFlowContactNormal=clamp(TR456_WATER_FLOW_CONTACT_NORMAL,0.0,3.0);
  float trshaderFlowRippleMix=clamp(TR456_WATER_FLOW_CONTACT_RIPPLES,0.0,1.0)*TR_TOGGLE_FLOW_CONTACTS;
  ...
  vec2 trshaderFlowSlope=trshaderFlowSlopeWorld*.54*trshaderSurfaceBreath+
    ...
    trshaderFlowDir*(trshaderPattern.w*.24+trshaderPattern.y*.16+trshaderSurfacePulse*.10+
      ...
      trshaderEdgeMicroWave*.055+trshaderEdgeMicroFoam*.018)+
    trshaderFlowSide*((trshaderPattern.x-trshaderPattern.z)*.075+
      ...
      (trshaderEdgeMicroA-trshaderEdgeMicroB)*trshaderEdgeMicroGate*.016)+
  ...
  float trshaderFlowBumpAmount=TR456_WATER_BUMP_STRENGTH*
    TR456_WATER_SYNTHETIC_BUMP_STRENGTH*TR456_WATER_FLOW_BUMP_STRENGTH*TR_TOGGLE_FLOW_DETAIL;
 ...
 float trshaderFlowSignal=trshaderSat(trshaderPattern.x*.42+trshaderPattern.y*.54+trshaderPattern.z*.34+
  ...
  vec2 trshaderCrossTear=trshaderFlowScreenSide*((trshaderPattern.x-trshaderPattern.z)*.0075+trshaderFlowSlope.y*.0042+
    ...
    (trshaderCrossPulse-.5)*.015*trshaderCrossStrength+trshaderCrossRefract*.0065);
  vec2 trshaderCrossWeave=trshaderFlowScreenSide*(trshaderCrossShear*.0048+trshaderCrossCounter*
    ...
    trshaderCrossStrength*trshaderCrossDistortion*.0018);
  ...
  float trshaderSceneScale=clamp(uTrWaterSyntheticMode.z,0.0,1.0);
  vec2 trshaderFlowWarp=(trshaderFlowScreenDir*(trshaderFlowSlope.x*.0052+trshaderRefrStreak.x*.50+trshaderMicroChop.x*.16)+
    trshaderFlowScreenSide*(trshaderRefrStreak.y*.40+trshaderMicroChop.y*.14+trshaderSmoothDeform.y*.020)+
   ...
   TR_TOGGLE_FLOW_WARP*mix(1.0,.10,trshaderSettleMask)*trshaderSceneScale;
 trshaderFlowWarp=trshaderSoftLimitVec2(trshaderFlowWarp,.062);
 ...
 float trshaderTintStrength=clamp(uTrWaterSyntheticInfo.y,0.0,2.0);
  float trshaderFlowReflectStrength=TR456_WATER_FLOW_REFLECTION*TR_TOGGLE_FLOW_REFLECTION;
  ...
  float trshaderDepthOpacity=trshaderDepthAwareOpacity(trshaderOpacity,trshaderFlowDepth);
   ...
   trshaderRefracted=mix(trshaderFlowTint+vec3(.004,.018,.022),
     trshaderRefracted,trshaderSceneScale);
 ...
 float trshaderFlowSpark=trshaderLineMask(trshaderSparkPhase,24.0)*
  ...
  float trshaderStreakGlint=(trshaderLineMask(trshaderFlowUv.x*4.35+trshaderValueNoise(trshaderFlowUv*3.2)*.24-
    ...
    TR456_WATER_FLOW_GLINT*TR_TOGGLE_FLOW_GLINT;
...
vec4 trshaderRenderCalmFlowSurface(TrshaderSyntheticFrame trshaderF, vec2 trshaderFlowDir, vec2 trshaderFlowSide,
 ...
 vec4 trshaderRefrStreak=trshaderCore.trshaderRefrStreak;
  vec4 trshaderVolumeWave=TR_TOGGLE_FLOW_TENSION>.001 ?
    trshaderFlowVolumeWaveField(trshaderFlowUv,trshaderGameTravel*.86,trshaderFlowSpeed,trshaderPatchField) : vec4(0.0);
  vec4 trshaderSmoothDeform=TR_TOGGLE_FLOW_DETAIL>.001 ?
    trshaderSmoothFlowDeformationField(vSynWorldPos,trshaderFlowDir,trshaderFlowSide,
      trshaderGameTravel*.86,trshaderFlowSpeed,.65) : vec4(0.0);
   float trshaderSettleMask=smoothstep(.18,.92,trshaderSettledWarp);
 float trshaderTensionStrength=clamp(TR456_WATER_FLOW_SURFACE_TENSION,0.0,2.0)*TR_TOGGLE_FLOW_TENSION;
 ...
 float trshaderCrossStrength=clamp(TR456_WATER_FLOW_CROSS_WAVE,0.0,2.0)*TR_TOGGLE_FLOW_TENSION;
  ...
  float trshaderCalmCrossRefract=(trshaderCalmCrossShear*.70+trshaderCalmCrossCounter*.30*
   ...
   vec2 trshaderCalmSlope=trshaderF.trshaderBaseField.xy*.48+trshaderF.trshaderRelief.xy*.26+trshaderF.trshaderAlive.xy*.34+
     ...
     (trshaderCalmCross2-.34)*.038*trshaderCrossStrength)+
   trshaderFlowSide*((trshaderPattern.x-trshaderPattern.z)*.030+trshaderMicroChop.y*.48+trshaderRefrStreak.y*.46+
     (pow(trshaderCalmCross,1.7)-.35)*.120*trshaderCrossStrength+
      ...
      (trshaderTensionLineB-.5)*trshaderTensionFilm*.055)+
 ...
 float trshaderFlowSignal=trshaderSat(trshaderPattern.x*.26+trshaderPattern.w*.18+abs(trshaderFlowField.z)*.25+
  ...
  vec2 trshaderCrossPull=trshaderFlowScreenSide*((trshaderPattern.x-trshaderPattern.z)*.0045+
    ...
    (trshaderCalmCross-.5)*.013*trshaderCrossStrength+trshaderCalmCrossRefract*.0048);
  vec2 trshaderCrossWeave=trshaderFlowScreenSide*(trshaderCalmCrossShear*.0038+
    ...
    trshaderFlowScreenDir*(trshaderCalmCrossCounter*trshaderCrossStrength*trshaderCrossDistortion*.0015);
  ...
  vec2 trshaderFlowWarp=(trshaderFlowScreenDir*(trshaderCalmSlope.x*.0058+trshaderRefrStreak.x*.54+
      trshaderMicroChop.x*.18)+trshaderFlowScreenSide*(trshaderRefrStreak.y*.44+
      trshaderMicroChop.y*.16+trshaderSmoothDeform.y*.016)+
   ...
   TR_TOGGLE_FLOW_WARP*mix(1.0,.12,trshaderSettleMask)*trshaderSceneScale;
 ...
 float trshaderDepthCue=0.0;
  ...
  float trshaderDepthOpacity=trshaderDepthAwareOpacity(trshaderOpacity,trshaderFloorDepth);
   ...
   trshaderRefracted=mix(trshaderTint+vec3(.004,.018,.022),
     trshaderRefracted,trshaderSceneScale);

 float trshaderReflectAmt=clamp(uTrWaterSyntheticInfo.z*TR456_WATER_FLOW_REFLECTION*
   TR456_WATER_REFLECT_STRENGTH*.16*TR_TOGGLE_FLOW_REFLECTION*clamp(uTrWaterSyntheticProfile.w,.05,1.0),
   0.0,1.0);
...
vec4 trshaderRenderStandingWater(TrshaderSyntheticFrame trshaderF){
 ...
 trshaderWarp+=(trshaderScreenDir*(trshaderUnderlay.y*.006+trshaderUnderlay.x*.0018)+
   trshaderScreenSide*(trshaderUnderlay.z*.005))*trshaderSceneScale;
 trshaderWarp=trshaderSoftLimitVec2(trshaderWarp,.043*trshaderSceneScale);
 ...
 float trshaderDepthCue=0.0;
  ...
  vec3 trshaderTint=mix(trshaderShallow,trshaderDeep,trshaderDepthCue)*trshaderTintStrength;
   vec3 trshaderSceneBase=texture(uTrWaterScene,clamp(trshaderF.trshaderScreen,vec2(.001),vec2(.999))).rgb;
   vec3 trshaderBaseScene=trshaderOriginalWaterGrade(trshaderSceneBase);
   vec3 trshaderWarpedScene=trshaderRefracted;
   vec3 trshaderOriginalUnderlay=trshaderOriginalUnderlayColor(trshaderF.trshaderScreen,trshaderBaseScene);
   float trshaderOriginalSignal=trshaderSat(length(trshaderOriginalUnderlay-trshaderBaseScene)*2.8+
     abs(trshaderLuma(trshaderOriginalUnderlay)-trshaderLuma(trshaderBaseScene))*2.2);
   float trshaderWarpDelta=clamp(dot(trshaderWarpedScene-trshaderBaseScene,vec3(.28,.52,.20)),-.060,.060);
   vec3 trshaderSyntheticFloor=trshaderTint*(.74+.18*trshaderOpacity)+vec3(.006,.014,.014);
   trshaderSyntheticFloor+=vec3(.006,.020,.024)*
     trshaderSat(trshaderF.trshaderStandingLife.z*.16+trshaderUnderlay.x*.12);
   vec3 trshaderWarpLens=mix(trshaderWarpedScene,
     trshaderBaseScene+vec3(trshaderWarpDelta)*vec3(.62,.78,.84),.06);
   vec3 trshaderOriginalBody=mix(trshaderWarpLens,
     trshaderOriginalUnderlay,.055+.065*trshaderOriginalSignal);
   float trshaderSyntheticFloorWeight=.020+.018*(1.0-trshaderSceneScale);
   trshaderRefracted=mix(trshaderOriginalBody,trshaderSyntheticFloor,trshaderSyntheticFloorWeight);
  ...
  float trshaderDepthBody=smoothstep(.16,.82,trshaderFloorDepth);
  float trshaderDepthOpacity=trshaderDepthAwareOpacity(trshaderOpacity,trshaderFloorDepth);
  float trshaderBaseMurk=trshaderSat(.055+trshaderOpacity*.18+trshaderFloorDepth*.18+
    (1.0-trshaderF.trshaderFresnel)*.055);
  ...
  trshaderRefracted=mix(trshaderRefracted,trshaderRefracted*vec3(.82,.92,.90)+trshaderMurkTint*.55,
    trshaderBaseMurk*.16);
 ...
 float trshaderCrest=trshaderSat(trshaderF.trshaderBaseField.z*.28+.30+trshaderF.trshaderAlive.z*.18)+
  ...
  trshaderTensionFilm*=trshaderUnderwaterDamp;
  float trshaderGlint=(trshaderRidgeA*.070+trshaderRidgeB*.056+trshaderRidgeCross*.150+trshaderReliefGrain*.052+
    ...
    trshaderFastPow58(trshaderSat(dot(trshaderF.trshaderNormal,normalize(vec3(-.28,.92,.26)))))*.14)*
    (0.22+trshaderF.trshaderFresnel*.54)*trshaderReflectAmt*TR456_WATER_GLINT_STRENGTH*
    clamp(TR456_WATER_SURFACE_CAUSTIC,0.0,2.0);
  trshaderGlint*=trshaderUnderwaterDamp;
 ...
 trshaderGlint+=trshaderSparkleMask*TR456_WATER_SPARKLE_STRENGTH*
   TR456_WATER_GLINT_STRENGTH*(.12+.28*trshaderReflectAmt);
 float trshaderContactLight=pow(trshaderSat(trshaderF.trshaderContacts.z+trshaderF.trshaderContactWake.z*.56),1.7)*
  ...
  trshaderWaterBase=mix(trshaderWaterBase,
    trshaderWaterBase*vec3(.88,.96,.94)+trshaderMurkTint*.24+vec3(.004,.008,.008),
    trshaderBaseMurk*.14);
  ...
  float trshaderOriginalLayerWeight=trshaderSat(.026+.050*trshaderOriginalSignal+
    .014*(1.0-trshaderF.trshaderFresnel));
  trshaderWaterBase=mix(trshaderWaterBase,
    trshaderOriginalUnderlay*vec3(.96,1.03,1.04)+trshaderTint*.025,
    trshaderOriginalLayerWeight);
  ...
  float trshaderReflectLuma=trshaderLuma(trshaderReflected);
  vec3 trshaderHighlightColor=mix(vec3(trshaderReflectLuma)*vec3(.90,.98,1.04),
    trshaderReflected,.46+trshaderReflectionMask*.32);
  trshaderHighlightColor=max(trshaderHighlightColor+vec3(.006,.010,.010),
    vec3(.045,.056,.054));
  vec3 trshaderSparkle=trshaderHighlightColor*(trshaderGlint*.62+trshaderContactLight*.74);
 ...
 trshaderCol+=vec3(.024,.058,.066)*trshaderRippleMemory*(.22+.34*(1.0-trshaderF.trshaderFresnel));
  ...
  float trshaderHaze=trshaderSat(trshaderFloorDepth*.14+(1.0-trshaderF.trshaderFresnel)*.040+
    trshaderMistLine*.006+trshaderOpacity*.045+trshaderBaseMurk*.060);
  ...
  trshaderCol=mix(trshaderCol,trshaderCol*vec3(.94,.98,.97)+trshaderHazeColor*.65,trshaderHaze*.18);
  trshaderCol+=trshaderHazeColor*(trshaderMistLine*.006+trshaderF.trshaderContacts.z*.003);
 float trshaderContrast=1.045;
 trshaderCol=(trshaderCol-.5)*trshaderContrast+.5;
 trshaderCol=clamp(trshaderCol,0.0,1.0);
  float trshaderStandingCompositeAlpha=clamp(.21+trshaderOpacity*.37+
    trshaderShoreFoam*.10+trshaderWakeFoam*.06+trshaderGlint*.10+trshaderContactLight*.12+
    trshaderReflectionMask*.070+trshaderHaze*.040+trshaderTensionFilm*.030+
    trshaderF.trshaderStandingLife.z*.022,.25,.62);
  trshaderStandingCompositeAlpha=mix(trshaderStandingCompositeAlpha,.39,trshaderBaseMurk*.10);
  trshaderStandingCompositeAlpha=max(trshaderStandingCompositeAlpha,
    mix(.31,.24,trshaderSceneScale));
...
void main(){
 ...
 if(trshaderSat(vSynFlowInfo.x)>.5 && TR_TOGGLE_FLOW_ENABLE<=.001) {
  trshaderFragColor=vec4(0.0);
  return;
 }
 ...
 float trshaderReflectMask=trshaderReflectOriginalMask(trshaderScreen);
 trshaderFragColor.a*=trshaderReflectMask;
 if(trshaderFragColor.a<=.001)
  discard;
}
