...
vec2 gameSurfaceDir(){
 return safeNormalize2(uParams.xy,safeNormalize2(vec2(.86,.50),vec2(1.0,0.0)));
}
...
vec3 contactMeshWave(vec3 worldPos, vec4 clip){
 ...
 for(int i=0;i<16;i++){
   ...
   if(screenContact>.5) {
     ...
     float useXY=clamp(step(1.5,mode)+(1.0-step(.5,mode))*autoXY,0.0,1.0);
     delta=mix(deltaXZ,deltaXY,useXY);
     vertical=1.0-smoothstep(radius*.22,radius*1.24,abs(worldPos.y-c.y));
   }
...
void emitSurfaceVertex(vec3 b){
 ...
 vec2 physicsDir=gameSurfaceDir();
 vec3 contact=contactMeshWave(worldPos,baseClip)*TR_TOGGLE_CONTACT_RIPPLES;
 vec3 breath=breathMeshWave(worldPos.xz,time,physicsDir);
 vec3 rain=rainMeshWave(worldPos.xz,time);
 vec3 facet=polygonFacetWave(worldPos.xz,physicsDir,time,.32);
 vec3 physics=physicsMeshWave(worldPos,baseClip,physicsDir,time);
 vec3 viewDir=normalize(-pos+normal*.0001);
 float distFade=1.0-smoothstep(5200.0,16800.0,length(pos));
 ...
 float breathAmp=30.0*clamp(TR456_WATER_CONTACT_MESH_STRENGTH,0.0,2.5)*
   distFade*angleFade*horizontal;
 float rainAmp=18.0*clamp(TR456_WATER_CONTACT_MESH_STRENGTH,0.0,2.5)*
   distFade*angleFade*horizontal;
 float facetAmp=22.0*clamp(TR456_WATER_CONTACT_MESH_STRENGTH,0.0,2.5)*
   distFade*angleFade*horizontal;
 float physicsAmp=46.0*clamp(TR456_WATER_CONTACT_MESH_STRENGTH,0.0,2.5)*
   distFade*angleFade*horizontal;
 float disp=(contact.z*contactAmp+breath.z*breathAmp+rain.z*rainAmp+
   facet.z*facetAmp+physics.z*physicsAmp)*
   edgeMask*TR_TOGGLE_MESH_DISPLACEMENT;
 pos+=heightDir*disp;
 worldPos+=heightDir*disp;
 ...
 vec2 meshSlope=contact.xy*(.030*clamp(TR456_WATER_CONTACT_NORMAL_STRENGTH,0.0,2.0))+
   breath.xy*.035+rain.xy*.008+
   facet.xy*.150*clamp(TR456_WATER_POLYGONAL_NORMAL,0.0,2.0)+
   physics.xy*.115*clamp(TR456_WATER_PHYSICS_NORMAL,0.0,2.0);
 meshSlope*=edgeMask*TR_TOGGLE_MESH_DISPLACEMENT;
...
void main(){
 ...
 for(int row=0;row<5;row++){
   ...
   for(int col=0;col<5;col++){
     ...
     emitSurfaceVertex(lower);
     emitSurfaceVertex(upper);
   }
   emitSurfaceVertex(vec3(0.0,float(width),float(row))/fn);
   EndPrimitive();
 }
}
