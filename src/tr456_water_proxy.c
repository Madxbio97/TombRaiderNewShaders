...
static const char *flow_inplace_fragment_shader_source(void) {
  return
    ...
    " float tension=trSat((tensionA*0.26+tensionB*0.24+abs(cross)*0.14)*uTrWaterFlowFx0.z*uTrWaterToggle1.z);\n"
...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    "void main(){\n"
    " vec2 tuv=vTexCoord;\n"
    " vec2 flow=normalize(vFlowDir+vec2(0.0003,0.0007));\n"
    " vec2 side=vec2(-flow.y,flow.x);\n"
    " float t=uTrWaterSyntheticInfo.w*(1.08+vSpeed*0.18);\n"
    " float travel=t*(1.75+vSpeed*0.25);\n"
    " vec2 tileEdge=abs(fract(tuv)-vec2(0.5))*2.0;\n"
    " float seamWidth=clamp(uTrWaterFlowFx3.y,0.006,0.20);\n"
    " float seamMask=max(smoothstep(1.0-seamWidth,1.0,tileEdge.x),smoothstep(1.0-seamWidth,1.0,tileEdge.y))*uTrWaterFlowFx3.x;\n"
    " seamMask=sat(seamMask)*0.18;\n"
    " vec2 texDrift=flow*(travel*0.024)+side*sin(travel*0.68+dot(vFlowUv,vec2(0.8,1.7)))*0.014;\n"
    " vec4 tex0=texture(sTex0_wrap,vec3(tuv.xy+texDrift,vLayer));\n"
    " vec4 tex1=texture(sTex0_wrap,vec3(tuv+texDrift+flow*0.029+side*0.009,vLayer));\n"
    " vec4 tex2=texture(sTex0_wrap,vec3(tuv+texDrift-flow*0.025+side*0.014,vLayer));\n"
    " vec4 tex3=texture(sTex0_wrap,vec3(tuv+texDrift+flow*0.010-side*0.021,vLayer));\n"
    " vec4 tex=mix(tex0,(tex0+tex1+tex2+tex3)*0.25,0.18+seamMask*0.18);\n"
    " if(uTrWaterSyntheticMode.x<0.5) tex=vec4(0.055,0.210,0.235,0.68);\n"
    " vec2 uv=vFlowUv*vec2(1.18,0.92);\n"
    " float swell=sin(uv.x*3.60+sin(uv.y*1.42+travel*0.28)*0.95-travel*1.05);\n"
    " float runA=sin(uv.x*8.20+uv.y*0.75-travel*2.15);\n"
    " float runB=sin(uv.x*13.4-uv.y*1.28-travel*3.05);\n"
    " float cross=sin(uv.y*7.40+uv.x*0.86+travel*0.95)*uTrWaterFlowFx0.w*uTrWaterToggle1.z;\n"
    " float ripple=sin(uv.x*25.0+uv.y*6.8-travel*6.20)*sin(uv.y*18.0-uv.x*2.6+travel*4.10)*uTrWaterFlowFx2.w*uTrWaterToggle1.x;\n"
    " float chop=sin(uv.x*17.0-uv.y*4.4-travel*4.70)*sin(uv.y*9.4+travel*2.40)*uTrWaterToggle1.x;\n"
    " float trembleA=sin(dot(uv,vec2(42.0,14.0))+travel*8.60)*0.58+sin(dot(uv,vec2(55.0,-31.0))+travel*10.20)*0.42;\n"
    " float trembleB=sin(dot(uv,vec2(-38.0,19.0))-travel*8.15)*0.62+sin(dot(uv,vec2(-52.0,-27.0))-travel*9.45)*0.38;\n"
    " float tremble=(trembleA*0.56+trembleB*0.50)*uTrWaterFlowFx2.w*uTrWaterToggle1.x;\n"
    " float trembleCross=(sin(dot(uv,vec2(6.2,49.0))+travel*9.35)*0.55+sin(dot(uv,vec2(-45.0,16.0))-travel*8.85)*0.45)*uTrWaterFlowFx2.w*uTrWaterToggle1.x;\n"
    " float trembleAbs=abs(tremble);\n"
    " float capFilm=smoothstep(0.34,0.88,abs(runA*0.50+cross*0.34+ripple*0.24+chop*0.18)+trembleAbs*0.10);\n"
    " float crest=smoothstep(0.44,0.94,swell*0.44+runA*0.30+runB*0.20+cross*0.24+ripple*0.14+chop*0.10+tremble*0.08+0.48);\n"
    " float flowBreak=smoothstep(0.28,0.88,abs(runA-runB)*0.42+abs(cross)*0.30+capFilm*0.24+abs(chop)*0.12+trembleAbs*0.08);\n"
    " float wave=swell*0.42+runA*0.26+cross*0.23+runB*0.11+ripple*0.09+chop*0.06+tremble*0.045;\n"
    " float textureGrain=sat((max(tex.r,max(tex.g,tex.b))-min(tex.r,min(tex.g,tex.b)))*2.5+tex.a*0.25);\n"
    " float streamJet=pow(sat(sin(uv.x*22.0+uv.y*1.55-travel*5.15)*0.5+0.5),7.0)*(0.35+0.65*sat(sin(uv.y*8.5+travel*1.65)*0.5+0.5));\n"
    " float tension=sat((capFilm*0.34+flowBreak*0.18+textureGrain*0.10)*uTrWaterFlowFx0.z*uTrWaterToggle1.z);\n"
    " float ridge=smoothstep(0.34,0.90,abs(wave)*0.98+crest*0.28+tension*0.22+textureGrain*0.10+trembleAbs*0.06)*uTrWaterFlowFx2.z;\n"
    " float mistWarp=sin(dot(uv,vec2(3.1,-1.7))+travel*0.37)*0.22+sin(dot(uv,vec2(-2.3,2.9))-travel*0.29)*0.18;\n"
    " float mistA=sin(uv.x*5.7+sin(uv.y*2.1-travel*0.31)*0.90+mistWarp-travel*1.18);\n"
    " float mistB=sin(uv.x*11.3+uv.y*0.9+mistWarp*1.7-travel*2.04);\n"
    " float mistC=sin(dot(uv,vec2(16.5,-3.8))+sin(dot(uv,vec2(-4.2,5.1))+travel*0.41)*0.55-travel*2.70);\n"
    " float flowMist=smoothstep(0.38,0.92,(mistA*0.44+mistB*0.31+mistC*0.25)*0.5+0.5);\n"
    " flowMist*=smoothstep(0.10,0.82,abs(cross)*0.22+capFilm*0.36+ridge*0.24+streamJet*0.18+trembleAbs*0.10)*uTrWaterToggle1.z;\n"
    " float reliefShade=wave*0.062+ridge*0.105+tension*0.052+flowMist*0.050+tremble*0.032-abs(cross)*0.016;\n"
    ...
    " vec2 warp=(dir*(wave*0.0190+tension*0.0110+ridge*0.0060+flowBreak*0.0020+streamJet*0.0080+flowMist*0.0070+tremble*0.0040)+sdir*(cross*0.0085+ripple*0.0040+chop*0.0032+mistC*0.0030+trembleCross*0.0032))*uTrWaterFlowFx0.y*uTrWaterFlowFx1.w*uTrWaterToggle0.y;\n"
    " vec2 shimmerWarp=dir*(ripple*0.011+chop*0.006+tension*0.006+streamJet*0.006+flowMist*0.0065+tremble*0.0050)+sdir*(cross*0.007+wave*0.002+mistB*0.0030+trembleCross*0.0040);\n"
    " vec3 scene=texture(uTrWaterScene,clamp(screen+warp+shimmerWarp*uTrWaterToggle0.y,vec2(0.001),vec2(0.999))).rgb;\n"
    " vec3 refl=texture(uTrWaterScene,clamp(screen+dir*(0.052+wave*0.026+tension*0.020+crest*0.014)+sdir*(cross*0.014+ripple*0.006+chop*0.004+0.007)+vec2(0.0,0.044),vec2(0.001),vec2(0.999))).rgb;\n"
    " float fres=sat(0.24+abs(wave)*0.18+tension*0.26+crest*0.10);\n"
    ...
    " vec3 originalBase=tex0.rgb*lit*vec3(0.76,0.92,0.97);\n"
    " originalBase=mix(originalBase,tex.rgb*lit*vec3(0.74,0.90,0.96),0.12);\n"
    " vec3 procBase=vec3(0.010,0.052,0.064)+vec3(0.008,0.030,0.038)*(swell*0.5+0.5)+vec3(0.006,0.018,0.022)*(ridge+tension*0.45);\n"
    " vec3 texWater=mix(vec3(texLuma),tex.rgb,0.52)*lit*vec3(0.80,1.03,1.10);\n"
    " vec3 tint=vec3(0.010,0.044,0.052)*uTrWaterSyntheticInfo.y;\n"
    " vec3 water=mix(originalBase,mix(procBase,texWater,0.58)+tint*(0.14+waterStrength*0.16),0.46);\n"
    " water=mix(water,scene*vec3(0.70,0.82,0.88)+tint*0.34,0.20*uTrWaterFlowFx0.y*uTrWaterToggle0.y);\n"
    " float reflAmt=sat((0.10+fres*0.28+ridge*0.07+glint*0.05)*uTrWaterSyntheticInfo.z*uTrWaterFlowFx0.x*uTrWaterFlowFx3.w*uTrWaterSyntheticProfile.w*uTrWaterToggle0.z);\n"
    ...
    " water+=vec3(0.007,0.020,0.022)*(ridge*uTrWaterFlowFx2.y+tension*uTrWaterFlowFx2.z)*uTrWaterToggle1.z;\n"
    " water+=vec3(0.058,0.086,0.074)*glint+vec3(0.010,0.022,0.022)*foam;\n"
    " water+=vec3(0.006,0.018,0.020)*(streamJet*0.90+ridge*0.40+flowMist*0.34+capFilm*0.12+textureGrain*0.14+trembleAbs*0.12)*waterStrength;\n"
    " water=mix(water,originalBase,0.18*uTrWaterToggle1.x*(1.0-seamMask*0.18));\n"
    " water+=clamp(originalBase-vec3(dot(originalBase,vec3(0.28,0.52,0.20))),vec3(-0.18),vec3(0.18))*0.12*(1.0-seamMask*0.25);\n"
    " water+=vec3(0.074,0.102,0.086)*glint*(0.48+ridge*0.26);\n"
    " water+=vec3(0.012,0.026,0.026)*foam*(0.55+flowBreak*0.18);\n"
    " water+=vec3(0.004,0.014,0.016)*(ridge*(0.22+capFilm*0.24)+flowMist*0.30)*uTrWaterFlowFx2.y;\n"
    " water=mix(water,water*0.84+vec3(0.014,0.040,0.044)+scene*vec3(0.055,0.068,0.074),0.11+ridge*0.040+streamJet*0.026+flowMist*0.155);\n"
    " water=mix(water,originalBase,0.10*uTrWaterToggle1.x*(1.0-seamMask*0.20));\n"
    " water=mix(vec3(0.010,0.030,0.034),water,vFog);\n"
    " float alpha=clamp(max(tex.a,0.42)*uTrWaterSyntheticInfo.x*(1.04+ridge*0.10+tension*0.10+streamJet*0.06),0.58,0.96)*uTrWaterToggle0.x;\n"
