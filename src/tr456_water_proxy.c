...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " float trembleCross=(sin(dot(uv,vec2(6.2,49.0))+travel*9.35)*0.62+sin(dot(uv,vec2(-45.0,16.0))-travel*8.85)*0.54)*uTrWaterFlowFx2.w*uTrWaterToggle1.x;\n"
    " float trembleAbs=abs(tremble);\n"
    " float breathA=sin(uv.x*1.95+sin(uv.y*1.15+travel*0.20)*0.80-travel*0.36);\n"
    " float breathB=sin(dot(uv,vec2(-1.20,2.05))+sin(dot(uv,vec2(0.70,-1.10))-travel*0.16)*0.55+travel*0.28);\n"
    ...
    " float mistWarp=sin(dot(uv,vec2(3.1,-1.7))+travel*0.37)*0.22+sin(dot(uv,vec2(-2.3,2.9))-travel*0.29)*0.18+breath*0.38;\n"
    ...
    " vec3 refl=texture(uTrWaterScene,clamp(screen+dir*(0.052+wave*0.028+tension*0.020+crest*0.014+breath*0.018)+sdir*(cross*0.014+ripple*0.006+chop*0.004+breathB*0.010+0.007)+vec2(0.0,0.044),vec2(0.001),vec2(0.999))).rgb;\n"
