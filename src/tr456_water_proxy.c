...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " float capFilm=smoothstep(0.34,0.88,abs(runA*0.50+cross*0.34+ripple*0.24+chop*0.18));\n"
    " float crest=smoothstep(0.44,0.94,swell*0.44+runA*0.30+runB*0.20+cross*0.24+ripple*0.14+chop*0.10+0.48);\n"
    " float flowBreak=smoothstep(0.28,0.88,abs(runA-runB)*0.42+abs(cross)*0.30+capFilm*0.24+abs(chop)*0.12);\n"
    " float wave=swell*0.42+runA*0.26+cross*0.23+runB*0.11+ripple*0.09+chop*0.06;\n"
    ...
    " float reliefShade=wave*0.050+ridge*0.075+tension*0.040-abs(cross)*0.018;\n"
    " float foam=smoothstep(0.66,0.985,crest*0.36+flowBreak*0.28+ridge*0.18+abs(chop)*0.08)*0.16*uTrWaterFlowFx1.z*uTrWaterToggle0.w*uTrWaterSyntheticProfile.y;\n"
    " float glint=pow(sat(capFilm*0.30+crest*0.28+tension*0.40+ridge*0.34+max(wave,0.0)*0.16+abs(chop)*0.08),15.0)*uTrWaterFlowFx1.x*uTrWaterToggle1.y;\n"
    " vec2 screen=gl_FragCoord.xy*max(uTrWaterCaptureInfo.xy,vec2(1.0/8192.0));\n"
    " vec2 dir=normalize(vec2(vFlowDir.x,-vFlowDir.y)+vec2(0.0001,0.0003));\n"
    " vec2 sdir=vec2(-dir.y,dir.x);\n"
    " vec2 warp=(dir*(wave*0.0160+tension*0.0100+crest*0.0075+ridge*0.0030+flowBreak*0.0035)+sdir*(cross*0.0080+ripple*0.0040+chop*0.0028))*uTrWaterFlowFx0.y*uTrWaterFlowFx1.w*uTrWaterToggle0.y;\n"
    " vec2 shimmerWarp=dir*(ripple*0.009+chop*0.005+tension*0.006)+sdir*(cross*0.007+wave*0.002);\n"
    ...
    " vec3 water=mix(procBase,texWater,0.66)+tint*(0.18+waterStrength*0.20);\n"
    " water=mix(water,scene*vec3(0.87,0.99,1.06)+tint*0.35,0.42*uTrWaterFlowFx0.y*uTrWaterToggle0.y);\n"
    ...
    " water+=vec3(0.085,0.110,0.090)*glint+vec3(0.018,0.034,0.032)*foam;\n"
    " water+=vec3(0.008,0.024,0.028)*(crest*0.45+capFilm*0.22+textureGrain*0.18)*waterStrength;\n"
    ...
    " water+=vec3(0.105,0.130,0.098)*glint*(0.55+crest*0.28);\n"
    " water+=vec3(0.022,0.040,0.036)*foam*(0.70+flowBreak*0.25);\n"
    " water+=vec3(0.012,0.038,0.046)*ridge*(0.24+capFilm*0.30)*uTrWaterFlowFx2.y;\n"
    " water=mix(vec3(0.010,0.030,0.034),water,vFog);\n"
    " float alpha=clamp(max(tex.a,0.35)*uTrWaterSyntheticInfo.x*(0.94+ridge*0.07+tension*0.10+crest*0.05),0.40,0.90)*uTrWaterToggle0.x;\n"
