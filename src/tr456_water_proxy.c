...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    "void main(){\n"
    " vec2 tuv=vTexCoord;\n"
    " vec4 tex=texture(sTex0_wrap,vec3(tuv.xy,vLayer));\n"
    ...
    " if(seamMask>0.001&&uTrWaterSyntheticMode.x>0.5){\n"
    "  vec2 fdir=normalize(vFlowDir+vec2(0.0003,0.0007));\n"
    "  vec2 fside=vec2(-fdir.y,fdir.x);\n"
    "  vec4 tx0=texture(sTex0_wrap,vec3(tuv+fdir*0.018,vLayer));\n"
    "  vec4 tx1=texture(sTex0_wrap,vec3(tuv-fdir*0.018,vLayer));\n"
    "  vec4 tx2=texture(sTex0_wrap,vec3(tuv+fside*0.012,vLayer));\n"
    "  vec4 seamTex=(tex+tx0+tx1+tx2)*0.25;\n"
    "  tex=mix(tex,seamTex,seamMask*0.68);\n"
    " }\n"
    ...
    " water=mix(water,scene*vec3(0.90,1.01,1.06)+refl*0.10+tint*0.65,seamMask*0.30*uTrWaterToggle0.y);\n"
