...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " vec2 texDrift=flow*(travel*0.024)+side*sin(dot(vFlowUv,vec2(0.8,1.7))-travel*0.68)*0.014;\n"
    ...
    " float along=uv.x;\n"
    " float across=uv.y;\n"
    " float swell=sin(along*3.60+sin(across*1.42-travel*0.28)*0.95-travel*1.05);\n"
    " float runA=sin(along*8.20+across*0.75-travel*2.15);\n"
    " float runB=sin(along*13.4-across*1.28-travel*3.05);\n"
    " float cross=sin(along*0.86+across*7.40-travel*0.95)*uTrWaterFlowFx0.w*uTrWaterToggle1.z;\n"
    " float ripple=sin(along*25.0+across*6.8-travel*6.20)*sin(along*2.6+across*18.0-travel*4.10)*uTrWaterFlowFx2.w*uTrWaterToggle1.x;\n"
    " float chop=sin(along*17.0-across*4.4-travel*4.70)*sin(along*1.1+across*9.4-travel*2.40)*uTrWaterToggle1.x;\n"
    " float trembleA=sin(along*42.0+across*14.0-travel*8.60)*0.58+sin(along*55.0-across*31.0-travel*10.20)*0.42;\n"
    " float trembleB=sin(along*38.0-across*19.0-travel*8.15)*0.62+sin(along*52.0+across*27.0-travel*9.45)*0.38;\n"
    " float tremble=(trembleA*0.68+trembleB*0.60)*uTrWaterFlowFx2.w*uTrWaterToggle1.x;\n"
    " float trembleCross=(sin(along*6.2+across*49.0-travel*9.35)*0.62+sin(along*45.0-across*16.0-travel*8.85)*0.54)*uTrWaterFlowFx2.w*uTrWaterToggle1.x;\n"
    " float trembleAbs=abs(tremble);\n"
    " float breathA=sin(along*1.95+sin(across*1.15-travel*0.20)*0.80-travel*0.36);\n"
    " float breathB=sin(along*1.20+across*2.05+sin(along*0.70-across*1.10-travel*0.16)*0.55-travel*0.28);\n"
    ...
    " float streamJet=pow(sat(sin(along*22.0+across*1.55-travel*5.15)*0.5+0.5),7.0)*(0.35+0.65*sat(sin(across*8.5-travel*1.65)*0.5+0.5));\n"
    ...
    " float mistWarp=sin(along*3.1-across*1.7-travel*0.37)*0.22+sin(along*2.3+across*2.9-travel*0.29)*0.18+breath*0.38;\n"
    " float mistA=sin(along*5.7+sin(across*2.1-travel*0.31)*0.90+mistWarp-travel*1.18);\n"
    " float mistB=sin(along*11.3+across*0.9+mistWarp*1.7-travel*2.04);\n"
    " float mistC=sin(along*16.5-across*3.8+sin(along*4.2+across*5.1-travel*0.41)*0.55-travel*2.70);\n"
