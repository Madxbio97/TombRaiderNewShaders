...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " float foam=smoothstep(0.46,0.94,crest*0.58+flowBreak*0.34+ridge*0.26+abs(chop)*0.12)*0.32*uTrWaterFlowFx1.z*uTrWaterToggle0.w*uTrWaterSyntheticProfile.y;\n"
    " float glint=pow(sat(capFilm*0.44+crest*0.38+tension*0.42+ridge*0.30+max(wave,0.0)*0.18+abs(chop)*0.10),9.0)*uTrWaterFlowFx1.x*uTrWaterToggle1.y;\n"
    " vec2 screen=gl_FragCoord.xy*max(uTrWaterCaptureInfo.xy,vec2(1.0/8192.0));\n"
    ...
    " water+=vec3(0.110,0.135,0.105)*glint+vec3(0.030,0.052,0.046)*foam;\n"
    ...
    " vec3 originalTop=mix(vec3(topLuma),tex0.rgb,0.86)*lit*vec3(0.82,1.02,1.08)+tint*0.16;\n"
    " float originalAlpha=sat((0.30+uTrWaterFlowFx2.w*0.24+tex0.a*0.08)*(1.0-seamMask*0.30))*uTrWaterToggle1.x;\n"
    " water=mix(water,originalTop,originalAlpha);\n"
    " water+=clamp(tex0.rgb-vec3(topLuma),vec3(-0.25),vec3(0.25))*0.16*(1.0-seamMask*0.45);\n"
    " water+=vec3(0.150,0.175,0.125)*glint*(0.65+crest*0.35);\n"
    " water+=vec3(0.045,0.070,0.058)*foam*(0.80+flowBreak*0.40);\n"
    " water+=vec3(0.018,0.050,0.058)*ridge*(0.30+capFilm*0.40)*uTrWaterFlowFx2.y;\n"
