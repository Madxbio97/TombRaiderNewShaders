...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " vec2 side=vec2(-flow.y,flow.x);\n"
    " float t=uTrWaterSyntheticInfo.w*(0.90+vSpeed*0.12);\n"
    " float travel=t*(1.18+vSpeed*0.14);\n"
    " vec2 tileEdge=abs(fract(tuv)-vec2(0.5))*2.0;\n"
    " float seamWidth=clamp(uTrWaterFlowFx3.y,0.006,0.20);\n"
    " float seamMask=max(smoothstep(1.0-seamWidth,1.0,tileEdge.x),smoothstep(1.0-seamWidth,1.0,tileEdge.y))*uTrWaterFlowFx3.x;\n"
    " seamMask=sat(seamMask);\n"
    " vec2 texDrift=flow*(travel*0.010)+side*sin(travel*0.42+dot(vFlowUv,vec2(0.8,1.7)))*0.006;\n"
    ...
    " vec2 uv=vFlowUv*vec2(1.0,0.78);\n"
    " float swell=sin(uv.x*2.85+sin(uv.y*1.18+travel*0.18)*0.78-travel*0.78);\n"
    " float runA=sin(uv.x*6.60+uv.y*0.62-travel*1.42);\n"
    " float runB=sin(uv.x*10.8-uv.y*1.15-travel*2.05);\n"
    " float cross=sin(uv.y*6.20+uv.x*0.70+travel*0.62)*uTrWaterFlowFx0.w*uTrWaterToggle1.z;\n"
    " float ripple=sin(uv.x*19.0+uv.y*5.6-travel*4.10)*sin(uv.y*15.0-uv.x*2.1+travel*2.85)*uTrWaterFlowFx2.w*uTrWaterToggle1.x;\n"
    " float capFilm=smoothstep(0.42,0.92,abs(runA*0.55+cross*0.32+ripple*0.18));\n"
    " float crest=smoothstep(0.52,0.96,swell*0.46+runA*0.30+runB*0.18+cross*0.20+ripple*0.10+0.45);\n"
    " float flowBreak=smoothstep(0.34,0.95,abs(runA-runB)*0.38+abs(cross)*0.26+capFilm*0.18);\n"
    " float wave=swell*0.46+runA*0.24+cross*0.20+runB*0.08+ripple*0.06;\n"
    ...
    " float foam=smoothstep(0.56,0.98,crest*0.55+flowBreak*0.30+ridge*0.22)*0.22*uTrWaterFlowFx1.z*uTrWaterToggle0.w*uTrWaterSyntheticProfile.y;\n"
    " float glint=pow(sat(capFilm*0.42+crest*0.34+tension*0.38+ridge*0.26+max(wave,0.0)*0.14),11.0)*uTrWaterFlowFx1.x*uTrWaterToggle1.y;\n"
    " vec2 screen=gl_FragCoord.xy*max(uTrWaterCaptureInfo.xy,vec2(1.0/8192.0));\n"
    " vec2 dir=normalize(vec2(vFlowDir.x,-vFlowDir.y)+vec2(0.0001,0.0003));\n"
    " vec2 sdir=vec2(-dir.y,dir.x);\n"
    " vec2 warp=(dir*(wave*0.0115+tension*0.0080+crest*0.0050+ridge*0.0025)+sdir*(cross*0.0064+ripple*0.0028))*uTrWaterFlowFx0.y*uTrWaterFlowFx1.w*uTrWaterToggle0.y;\n"
    " vec3 scene=texture(uTrWaterScene,clamp(screen+warp,vec2(0.001),vec2(0.999))).rgb;\n"
    " vec3 refl=texture(uTrWaterScene,clamp(screen+dir*(0.046+wave*0.020+tension*0.016+crest*0.010)+sdir*(cross*0.011+ripple*0.004+0.006)+vec2(0.0,0.040),vec2(0.001),vec2(0.999))).rgb;\n"
    ...
    " float originalAlpha=sat((0.34+uTrWaterFlowFx2.w*0.28+tex0.a*0.10)*(1.0-seamMask*0.30))*uTrWaterToggle1.x;\n"
