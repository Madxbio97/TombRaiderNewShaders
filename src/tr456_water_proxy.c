...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " vec4 tex0=texture(sTex0_wrap,vec3(tuv.xy,vLayer));\n"
    " vec4 tex1=texture(sTex0_wrap,vec3(tuv+flow*0.031+side*0.007,vLayer));\n"
    " vec4 tex2=texture(sTex0_wrap,vec3(tuv-flow*0.027+side*0.015,vLayer));\n"
    " vec4 tex3=texture(sTex0_wrap,vec3(tuv+flow*0.011-side*0.023,vLayer));\n"
    " vec4 tex=mix(tex0,(tex0+tex1+tex2+tex3)*0.25,0.36+seamMask*0.58);\n"
    " if(uTrWaterSyntheticMode.x<0.5) tex=vec4(0.055,0.210,0.235,0.68);\n"
    " float t=uTrWaterSyntheticInfo.w*(0.88+vSpeed*0.10);\n"
    " vec2 uv=vFlowUv*vec2(1.0,0.78);\n"
    " float travel=t*(0.96+vSpeed*0.10);\n"
    " float swell=sin(uv.x*3.30+sin(uv.y*1.35+travel*0.16)*0.70-travel*0.58);\n"
    " float ribbon=sin(uv.x*9.20+uv.y*0.58-travel*1.32);\n"
    " float ribbon2=sin(uv.x*15.0-uv.y*0.92-travel*1.95);\n"
    " float cross=sin(uv.y*5.60+uv.x*0.82+travel*0.44)*uTrWaterFlowFx0.w*uTrWaterToggle1.z;\n"
    " float micro=sin(uv.x*27.0+uv.y*3.25-travel*3.65)*uTrWaterFlowFx2.w*uTrWaterToggle1.x;\n"
    " float streamLine=max(trLine(uv.x*5.8+uv.y*0.22-travel*0.66,5.4),trLine(uv.x*9.6-uv.y*0.34-travel*1.05,8.0));\n"
    " float wave=swell*0.44+ribbon*0.28+cross*0.18+ribbon2*0.07+micro*0.03;\n"
    " float textureGrain=sat((max(tex.r,max(tex.g,tex.b))-min(tex.r,min(tex.g,tex.b)))*2.5+tex.a*0.25);\n"
    " float tension=sat((streamLine*0.30+abs(cross)*0.20+textureGrain*0.12)*uTrWaterFlowFx0.z*uTrWaterToggle1.z);\n"
    " float ridge=smoothstep(0.42,0.94,abs(wave)+streamLine*0.22+textureGrain*0.10)*uTrWaterFlowFx2.z;\n"
    " float foam=smoothstep(0.70,0.98,streamLine+abs(cross)*0.16)*0.10*uTrWaterFlowFx1.z*uTrWaterToggle0.w*uTrWaterSyntheticProfile.y;\n"
    " float glint=pow(sat(streamLine*0.72+tension*0.44+ridge*0.28+max(wave,0.0)*0.16),10.0)*uTrWaterFlowFx1.x*uTrWaterToggle1.y;\n"
    " vec2 screen=gl_FragCoord.xy*max(uTrWaterCaptureInfo.xy,vec2(1.0/8192.0));\n"
    " vec2 dir=normalize(vec2(vFlowDir.x,-vFlowDir.y)+vec2(0.0001,0.0003));\n"
    " vec2 sdir=vec2(-dir.y,dir.x);\n"
    " vec2 warp=(dir*(wave*0.0095+tension*0.0070+streamLine*0.0040+ridge*0.0022)+sdir*(cross*0.0058+micro*0.0015))*uTrWaterFlowFx0.y*uTrWaterFlowFx1.w*uTrWaterToggle0.y;\n"
    " vec3 scene=texture(uTrWaterScene,clamp(screen+warp,vec2(0.001),vec2(0.999))).rgb;\n"
    " vec3 refl=texture(uTrWaterScene,clamp(screen+dir*(0.040+wave*0.016+tension*0.014+streamLine*0.007)+sdir*(cross*0.010+0.008)+vec2(0.0,0.038),vec2(0.001),vec2(0.999))).rgb;\n"
    " float fres=sat(0.22+abs(wave)*0.16+tension*0.24+streamLine*0.10);\n"
    ...
    " vec3 procBase=vec3(0.026,0.135,0.158)+vec3(0.018,0.064,0.076)*(swell*0.5+0.5)+vec3(0.010,0.034,0.038)*streamLine;\n"
    ...
    " water+=vec3(0.012,0.030,0.034)*(streamLine+textureGrain*0.35)*waterStrength;\n"
    " water=mix(water,procBase+scene*vec3(0.28,0.38,0.40)+refl*0.13+tint*0.75,seamMask*0.42*uTrWaterToggle0.y);\n"
    " water=mix(vec3(0.010,0.030,0.034),water,vFog);\n"
    " float alpha=clamp(max(tex.a,0.35)*uTrWaterSyntheticInfo.x*(0.96+ridge*0.07+tension*0.10+streamLine*0.05),0.42,0.92)*uTrWaterToggle0.x;\n"
