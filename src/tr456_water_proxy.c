...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " water=mix(water,scene*vec3(0.72,0.88,0.96)+tint*0.55,0.26*uTrWaterFlowFx0.y*uTrWaterToggle0.y);\n"
    ...
    " water+=vec3(0.018,0.052,0.060)*(ridge*uTrWaterFlowFx2.y+tension*uTrWaterFlowFx2.z)*uTrWaterToggle1.z;\n"
    " water+=vec3(0.058,0.086,0.074)*glint+vec3(0.010,0.022,0.022)*foam;\n"
    " water+=vec3(0.010,0.032,0.038)*(streamJet*0.90+ridge*0.32+capFilm*0.12+textureGrain*0.14+trembleAbs*0.12)*waterStrength;\n"
    " vec3 originalTop=mix(vec3(topLuma),tex0.rgb,0.92)*lit*vec3(0.64,0.88,0.98)+tint*0.08;\n"
    " originalTop=min(originalTop,water+vec3(0.070,0.115,0.125));\n"
    " float originalAlpha=sat((0.26+uTrWaterFlowFx2.w*0.20+tex0.a*0.05)*(1.0-seamMask*0.18))*uTrWaterToggle1.x;\n"
    " water=mix(water,originalTop,originalAlpha);\n"
    " water+=clamp(tex0.rgb-vec3(topLuma),vec3(-0.22),vec3(0.22))*0.22*(1.0-seamMask*0.25);\n"
    " water+=vec3(0.074,0.102,0.086)*glint*(0.48+ridge*0.26);\n"
    " water+=vec3(0.012,0.026,0.026)*foam*(0.55+flowBreak*0.18);\n"
    " water+=vec3(0.012,0.038,0.046)*ridge*(0.24+capFilm*0.30)*uTrWaterFlowFx2.y;\n"
    " water=mix(water,water*0.82+vec3(0.020,0.060,0.070)+scene*vec3(0.06,0.08,0.09),0.16+ridge*0.05+streamJet*0.04);\n"
