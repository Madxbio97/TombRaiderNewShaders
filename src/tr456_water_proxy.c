...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " float cross=sin(a*0.86+b*7.40-travel*0.95+patternWarpA*0.35)*uTrWaterFlowFx0.w*uTrWaterToggle1.z;\n"
    ...
    " vec3 refl=refl0*0.42+refl1*0.34+refl2*0.24;\n"
    " float reflLum=dot(refl,vec3(0.28,0.52,0.20));\n"
    " float reflHi=smoothstep(0.24,0.82,reflLum);\n"
    " refl=mix(vec3(reflLum)*vec3(0.78,0.90,0.96)+vec3(0.010,0.030,0.034),refl,0.24+reflHi*0.32);\n"
    " refl=mix(refl,scene*vec3(0.68,0.80,0.86)+vec3(0.010,0.030,0.034),0.16+flowMist*0.10);\n"
    ...
    " float reflAmt=sat((0.045+fres*0.18+ridge*0.045+glint*0.035+flowMist*0.040)*uTrWaterSyntheticInfo.z*uTrWaterFlowFx0.x*uTrWaterFlowFx3.w*uTrWaterSyntheticProfile.w*uTrWaterToggle0.z);\n"
    " water=mix(water,refl*vec3(0.90,0.98,1.03)+tint*0.18,reflAmt);\n"
