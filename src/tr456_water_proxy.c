...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " float ridge=smoothstep(0.40,0.94,abs(wave)*0.90+crest*0.24+tension*0.20+textureGrain*0.08)*uTrWaterFlowFx2.z;\n"
    " float reliefShade=wave*0.050+ridge*0.075+tension*0.040+tremble*0.028-abs(cross)*0.018;\n"
    ...
    " vec2 warp=(dir*(wave*0.0175+tension*0.0100+ridge*0.0045+flowBreak*0.0020+streamJet*0.0080+tremble*0.0040)+sdir*(cross*0.0080+ripple*0.0040+chop*0.0032+trembleCross*0.0032))*uTrWaterFlowFx0.y*uTrWaterFlowFx1.w*uTrWaterToggle0.y;\n"
    " vec2 shimmerWarp=dir*(ripple*0.011+chop*0.006+tension*0.006+streamJet*0.006+tremble*0.0050)+sdir*(cross*0.007+wave*0.002+trembleCross*0.0040);\n"
    ...
    " float topLuma=dot(tex0.rgb,vec3(0.28,0.52,0.20));\n"
    ...
    " vec3 water=mix(procBase,texWater,0.78)+tint*(0.22+waterStrength*0.24);\n"
    " water=mix(water,scene*vec3(0.70,0.82,0.88)+tint*0.36,0.24*uTrWaterFlowFx0.y*uTrWaterToggle0.y);\n"
    ...
    " water+=vec3(0.006,0.018,0.020)*(streamJet*0.90+ridge*0.32+capFilm*0.12+textureGrain*0.14+trembleAbs*0.12)*waterStrength;\n"
    " vec3 originalTop=mix(vec3(topLuma),tex0.rgb,0.98)*lit*vec3(0.72,0.90,0.96);\n"
    " originalTop=mix(originalTop,tex.rgb*lit*vec3(0.74,0.92,0.98),0.18);\n"
    " float originalAlpha=sat((0.34+uTrWaterFlowFx2.w*0.22+tex0.a*0.06)*(1.0-seamMask*0.16))*uTrWaterToggle1.x;\n"
    " water=mix(water,originalTop,originalAlpha);\n"
    " water+=clamp(tex0.rgb-vec3(topLuma),vec3(-0.22),vec3(0.22))*0.30*(1.0-seamMask*0.25);\n"
    " water+=vec3(0.074,0.102,0.086)*glint*(0.48+ridge*0.26);\n"
    " water+=vec3(0.012,0.026,0.026)*foam*(0.55+flowBreak*0.18);\n"
    " water+=vec3(0.004,0.014,0.016)*ridge*(0.18+capFilm*0.22)*uTrWaterFlowFx2.y;\n"
    " water=mix(water,water*0.86+vec3(0.012,0.038,0.044)+scene*vec3(0.05,0.065,0.075),0.12+ridge*0.035+streamJet*0.030);\n"
    " vec3 originalFinal=mix(vec3(topLuma),tex0.rgb,1.0)*lit*vec3(0.76,0.92,0.97);\n"
    " float originalFinalAlpha=sat((0.24+uTrWaterFlowFx2.w*0.30+tex0.a*0.08)*(1.0-seamMask*0.22))*uTrWaterToggle1.x;\n"
    " water=mix(water,originalFinal,originalFinalAlpha);\n"
