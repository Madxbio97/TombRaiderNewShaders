...
static char *flow_lite_fragment_shader(void) {
  return trshader_dup_text(
    ...
    " vec4 tex=mix(tex0,(tex0+tex1+tex2+tex3)*0.25,0.24+seamMask*0.34);\n"
    ...
    " vec3 procBase=vec3(0.024,0.125,0.150)+vec3(0.020,0.070,0.082)*(swell*0.5+0.5)+vec3(0.010,0.032,0.036)*(crest+capFilm*0.35);\n"
    ...
    " vec3 water=mix(procBase,texWater,0.50-seamMask*0.24)+tint*(0.26+waterStrength*0.28);\n"
    " water=mix(water,scene*vec3(0.91,1.01,1.06)+tint,0.30*uTrWaterFlowFx0.y*uTrWaterToggle0.y);\n"
    ...
    " water+=vec3(0.012,0.032,0.036)*(crest*0.70+capFilm*0.35+textureGrain*0.24)*waterStrength;\n"
    " water=mix(water,procBase+scene*vec3(0.18,0.26,0.28)+refl*0.08+tint*0.55,seamMask*0.18*uTrWaterToggle0.y);\n"
    " vec3 originalTop=mix(vec3(topLuma),tex0.rgb,0.70)*lit*vec3(0.74,0.96,1.02)+tint*0.12;\n"
    " float originalAlpha=sat((0.16+uTrWaterFlowFx2.w*0.15+tex0.a*0.05)*(1.0-seamMask*0.35))*uTrWaterToggle1.x;\n"
    " water=mix(water,originalTop,originalAlpha);\n"
    " water+=clamp(tex0.rgb-vec3(topLuma),vec3(-0.25),vec3(0.25))*0.10*(1.0-seamMask*0.50);\n"
